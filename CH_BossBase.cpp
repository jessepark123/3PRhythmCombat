...
TArray<int> ACH_BossBase::FindSuitableAttacksForRhythm(TArray<EQuartzCommandQuantization> InAttackTimings)
{
	TArray<int> returnedPossibleAttackIndeces;

	for (FCustomAttackNotes noteStruct : CustomAttackNotesList)
	{
		if (noteStruct.NotesInCustomAttack == InAttackTimings)
		{
			//adds index from the customattacknoteslist array into the returned possible attack indeces array, so that UseCustomAttackStruct can use it *use the value of the ints in this array NOT the index
			//which index to choose from the returnedpossibleattackindeces array is determined by AI at BP level
			returnedPossibleAttackIndeces.Add(CustomAttackNotesList.Find(noteStruct));
		}
	}
	return returnedPossibleAttackIndeces;
}
